...
/*   Updated: 2025/07/02 23:45:21 by dbank            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void move_player(struct mlx_key_data key, t_data *game)
{
	...
	{
		new_y = game->player.pos_y + sin(game->player.orientation) / 3;
		new_x = game->player.pos_x + cos(game->player.orientation) / 3;
	}
...
void render_background(const int ceilingc[3], const int floorc[3], t_mlx *mlx)
{
	...
	fill_image(mlx->ceiling, converted_ceiling, 800, 300);
	mlx->floor = mlx_new_image(mlx->mlx, 800, 500);
	fill_image(mlx->floor, converted_floor, 800, 500);
